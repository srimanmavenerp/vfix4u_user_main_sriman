...
class LoyaltyPointController extends GetxController implements GetxService {
  ...
  Future<void> convertLoyaltyPoint() async {
    ...
    update();
    Response response =
        await loyaltyPointRepo.convertLoyaltyPoint(loyaltyPointController.text);
    if (response.statusCode == 200) {
      loyaltyPointController.text = '';
      await getLoyaltyPointData(1);
      Get.back();
      customSnackBar("point_converted_to_wallet_money".tr,
  ...
  Future<void> getLoyaltyPointData(int offset, {reload = false}) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (offset != 1) {
        listOfTransaction
            .addAll(loyaltyPointModel!.content!.transactions!.data!);
      } else {
        listOfTransaction = [];
        listOfTransaction
            .addAll(loyaltyPointModel!.content!.transactions!.data!);

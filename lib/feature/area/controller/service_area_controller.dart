...
class ServiceAreaController extends GetxController implements GetxService {
  ...
  Future<void> getZoneList(
      {Map<String, GlobalKey>? globalKeyMap, bool reload = true}) async {
    ...
    DataSyncHelper.fetchAndSyncData(
      ...
      onResponse: (data, source) {
        ...
        data['content']['data']
            .forEach((zone) => _zoneList!.add(ZoneModel.fromJson(zone)));
  ...
  Future<void> setMarker(
      List<ZoneModel> zoneList, Map<String, GlobalKey> globalKeymap) async {
    ...
    for (int index = 0; index < zoneList.length; index++) {
      ...
      markerList.add(Marker(
        ...
        position:
            computeCentroid(coordinates: zoneList[index].formattedCoordinates!),
      ));
    }
  ...
  LatLng computeCentroid(
      {List<Coordinates>? coordinates, Iterable<LatLng>? points}) {
    double latitude = 0;
    double longitude = 0;
    int n = 1;

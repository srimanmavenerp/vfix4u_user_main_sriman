import 'package:Vfix4u/feature/search/model/filter_model.dart';
import 'package:Vfix4u/feature/search/model/search_service_model.dart';
import 'package:Vfix4u/feature/search/model/search_suggestion_model.dart';
...
class AllSearchController extends GetxController implements GetxService {
  ...
  Future<void> populatedSearchController(String queryText,
  ...
  Future<void> searchData(
    ...
    if (response.statusCode == 200) {
      ...
      if (_searchServiceList != null && offset != 1) {
        _searchServiceList!
            .addAll(_serviceModel?.content?.servicesContent?.serviceList ?? []);
      } else {
        _searchServiceList = [];
        _searchServiceList!
            .addAll(_serviceModel?.content?.servicesContent?.serviceList ?? []);
  ...
  Future<void> getSearchSuggestion(
    ...
    Response response =
        await searchRepo.getSearchSuggestion(query: searchController.text);
    if (response.statusCode == 200) {
  ...
  updateIsFavoriteValue(int status, String serviceId,
    ...
    if (_searchServiceList != null) {
      int? index =
          _searchServiceList?.indexWhere((element) => element.id == serviceId);
  ...
  updatedIsSortedAppliedStatus({bool shouldUpdate = true}) {
    ...
    if (_selectedSortBy != null) {
      sortedByList
          .add(FilterModel(title: _selectedSortBy, type: AllFilterType.sortBy));
    }
    if (_selectedSortByType != "default") {
      sortedByList.add(FilterModel(
          title: _selectedSortByType, type: AllFilterType.sortByType));
    }
  ...
  updatedIsFilteredAppliedStatus({bool shouldUpdate = true}) {
    ...
    if ((_filteredMinPrice != null &&
      ...
      filteredByList
          .add(FilterModel(title: "price", type: AllFilterType.price));
    }
    if (_selectedRating != null) {
      filteredByList
          .add(FilterModel(title: "rating", type: AllFilterType.rating));
    }

    if (_selectedCategoryId.isNotEmpty) {
      for (var element in _selectedCategoryId) {
        filteredByList
            .add(FilterModel(title: "category", type: AllFilterType.category));

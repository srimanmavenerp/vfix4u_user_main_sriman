...
class ConversationController extends GetxController
    ...
    messageScrollController.addListener(() {
      if (messageScrollController.position.pixels ==
        ...
        if (_messageOffset! < _messagePageSize!) {
          getConversation(_channelId, _messageOffset! + 1,
  ...
  Future<void> pickMultipleImage(bool isRemove, {int? index}) async {
    ...
    if (isRemove) {
      ...
      for (var element in pickImages) {
        if (await ImageSize.getImageSizeFromXFile(element) >
            AppConstants.maxSizeOfASingleFile) {
          _singleFIleCrossMaxLimit = true;
        } else {
          if ((await ImageSize.getMultipleImageSizeFromXFile(
                          _pickedImageFiles!) +
                      await ImageSize.getImageSizeFromXFile(element)) <
                  AppConstants.maxLimitOfFileSentINConversation &&
              _pickedImageFiles!.length <
                  AppConstants.maxLimitOfTotalFileSent) {
      ...
      if (_pickedImageFiles?.length == AppConstants.maxLimitOfTotalFileSent &&
          await ImageSize.getMultipleImageSizeFromXFile(pickImages) >
              AppConstants.maxLimitOfFileSentINConversation) {
  ...
  Future<void> getChannelList(int offset,
      {bool isFromPagination = false,
      bool reload = false,
      bool isFirst = false,
      String type = "provider"}) async {
    ...
    Response response =
        await conversationRepo.getChannelList(offset, type: type);
    ...
    if (response.statusCode == 200) {
      ...
      if (response.body['content']['adminChannel'] != null) {
        _adminConversation =
            ChannelData.fromJson(response.body['content']['adminChannel']);
      }
    } else {
  ...
  Future<void> getSearchedChannelList({
    String? query,
  }) async {
    ...
    Response response =
        await conversationRepo.searchChannelList(queryText: query);
  ...
  Future<void> createChannel(String userID, String referenceID,
    ...
    Response response =
        await conversationRepo.createChannel(userID, referenceID);
    if (response.statusCode == 200) {
      if (fromBookingDetailsPage) {
        Get.back();
        Get.toNamed(RouteHelper.getChatScreenRoute(
            response.body['content']['id'], name, image, phone, userType));
      } else {
        Get.toNamed(RouteHelper.getChatScreenRoute(
            response.body['content']['id'], name, image, phone, userType));
      }
  ...
  Future<void> getConversation(String channelID, int offset,
      {bool isFromPagination = false, bool isInitial = false}) async {
    if (!isFromPagination && isInitial) {
      _conversationList = null;
    }
    _messageOffset = offset;
    Response response =
        await conversationRepo.getConversation(channelID, offset);
    if (response.statusCode == 200) {
      if (!isFromPagination) {
        _conversationList = [];
      }
  ...
  String getChatTime(String todayChatTimeInUtc, String? nextChatTimeInUtc) {
    ...
    DateTime todayConversationDateTime =
        DateConverter.isoUtcStringToLocalDate(todayChatTimeInUtc);
    ...
    if (nextChatTimeInUtc == null) {
      ...
      nextConversationDateTime =
          DateConverter.isoUtcStringToLocalDate(nextChatTimeInUtc);
      if (kDebugMode) {
  ...
  String getChatTimeWithPrevious(
      ConversationData currentChat, ConversationData? previousChat) {
    DateTime todayConversationDateTime =
        DateConverter.isoUtcStringToLocalDate(currentChat.createdAt ?? "");
  ...
  bool isSameUserWithPreviousMessage(ConversationData? previousConversation,
  ...
  bool isSameUserWithNextMessage(ConversationData? currentConversation,

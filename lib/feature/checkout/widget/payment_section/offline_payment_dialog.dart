...
class OfflinePaymentDialog extends StatefulWidget {
  ...
  const OfflinePaymentDialog(
      {super.key, required this.totalAmount, required this.index});
...
class _OfflinePaymentDialogState extends State<OfflinePaymentDialog> {
  ...
  void initState() {
    ...
    scrollController!
        .scrollToIndex(widget.index, preferPosition: AutoScrollPosition.middle);
  ...
  Widget build(BuildContext context) {
    List<OfflinePaymentModel> offlinePaymentMethodList =
        Get.find<CheckOutController>().offlinePaymentModelList;
    return Center(
      child: Container(
          ...
          return Column(children: [
            Expanded(
              ...
              child: Column(children: [
                  ...
                  child: Row(
                      children: offlinePaymentMethodList
                          .map((offline) => AutoScrollTag(
                                controller: scrollController!,
                                key: ValueKey(
                                    offlinePaymentMethodList.indexOf(offline)),
                                index:
                                    offlinePaymentMethodList.indexOf(offline),
                                child: InkWell(
                                  onTap: () async {
                                    ...
                                    child: Container(
                                      ...
                                      child: Column(children: [
                                        ...
                                        const SizedBox(
                                            height:
                                                Dimensions.paddingSizeDefault),
                                        if (offline.paymentInformation != null)
                                          BillInfoWidget(
                                              methodList:
                                                  offline.paymentInformation),
                                      ]),
                                    ),
                                  ),
                                ),
                ...
                const SizedBox(height: Dimensions.paddingSizeSmall),
                Text(
                  '${'amount'.tr} : ${PriceConverter.convertPrice(widget.totalAmount)}',
                  style:
                      robotoBold.copyWith(fontSize: Dimensions.fontSizeLarge),
                ),
                const SizedBox(height: Dimensions.paddingSizeExtraLarge),
                if (checkoutController
                        .selectedOfflineMethod?.paymentInformation !=
                    null)
                  PaymentInfoWidget(
                      methodInfo: checkoutController
                          .selectedOfflineMethod!.customerInformation),
                SizedBox(
              ...
              CustomButton(
                ...
                onPressed: () {
                  if (checkoutController.formKey.currentState!.validate()) {
                    ...
                    checkoutController.offlinePaymentInputFieldValues
                        .add(value);
        ...
        children: methodList!
            .map((method) => Padding(
                  ...
                  child: Row(
                      ...
                      children: [
                        Text(
                          method.title?.replaceAll("_", " ").capitalizeFirst ??
                              '',
                          style: robotoRegular,

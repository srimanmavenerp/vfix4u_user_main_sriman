...
class CreatePostController extends GetxController implements GetxService {
  ...
  Future<void> createCustomPost(String? schedule,
      {AddressModel? serviceAddress}) async {
    _isLoading = true;
    update();
    AddressModel? addressModel = Get.find<LocationController>().selectedAddress;

    Response response = await createPostRepo.createCustomPost(CreatePostBody(
    ...
    if (response.statusCode == 200) {
      ...
      customSnackBar("your_post_has_been_created_successfully".tr,
  ...
  Future<void> getProvidersOfferList(int offset, String postId,
      {reload = false}) async {
    ...
    Response response =
        await createPostRepo.getProvidersOfferList(offset, postId);
  ...
  Future<void> providerBidDetailsForNotification(
      String postId, String providerId) async {
    Response response =
        await createPostRepo.getProviderBidDetails(postId, providerId);
    if (response.statusCode == 200) {
      if (response.body['content'] != null) {
        notificationBiddingDetails =
            ProviderOfferData.fromJson(response.body['content']['post_bid']);
  ...
  Future<Response> updatePostStatus(
      String postId, String providerId, String status,
      {int? isPartial,
      String? serviceAddress,
      String? serviceAddressId}) async {
    Response response = await createPostRepo.updatePostStatus(
        postId, providerId, status,
        isPartial: isPartial,

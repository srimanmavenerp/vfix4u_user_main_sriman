...
class ProviderBookingController extends GetxController implements GetxService {
  ...
  String formatDays(List<String> dayList) {
    ...
    List<String> formattedList =
        dayList.map((day) => day[0].toLowerCase() + day.substring(1)).toList();
    ...
    if (offset != 1 || _providerList == null || reload) {
      ...
      if (offset == 1) {
        await DataSyncHelper.fetchAndSyncData(
          fetchFromLocal: () => providerBookingRepo
              .getProviderList<CacheResponseData>(offset, body,
                  source: DataSourceEnum.local),
          fetchFromClient: () => providerBookingRepo
              .getProviderList(offset, body, source: DataSourceEnum.client),
          onResponse: (data, source) {
            _providerModel = ProviderModel.fromJson(data);
            _providerList = [];
            _providerList!
                .addAll(ProviderModel.fromJson(data).content?.data ?? []);
            _calculateDistance();
            update();
          },
        );
      } else {
        ApiResponseModel response = await providerBookingRepo
            .getProviderList(offset, body, source: DataSourceEnum.client);
    ...
    _providerList?.forEach((element) {
      double distance =
          MapHelper.getDistanceBetweenUserCurrentLocationAndProvider(
              Get.find<LocationController>().getUserAddress()!, element);
      element.distance = distance;
    });
  }

  Future<void> getProviderDetailsData(String providerId, bool reload,
      {int offSet = 1}) async {
    if (_providerDetailsContent == null || reload) {
      ...
      Response response =
          await providerBookingRepo.getProviderDetails(providerId, offSet);
      if (response.statusCode == 200) {
        _providerDetailsContent =
            ProviderDetails.fromJson(response.body).content;
        ...
        if (offSet != 1) {
          _providerDetailsContent?.providerReview?.reviewList
              ?.forEach((element) {
  ...
  Future<void> updateIsFavoriteStatus(
      {required String providerId, int? index}) async {
    _apiHitCount++;
    Response response =
        await providerBookingRepo.updateIsFavoriteStatus(serviceId: providerId);
  ...
  updateProviderIsFavoriteValue(int status, String providerId,
      {bool shouldUpdate = false, bool fromProviderBooking = true}) {
    int? index =
        _providerList?.indexWhere((element) => element.id == providerId);
    if (index != null && index > -1) {
      _providerList?[index].isFavorite = status;
    }
    Get.find<AdvertisementController>()
        .updateIsFavoriteValue(status, providerId, shouldUpdate: true);
    if (fromProviderBooking) {
      Get.find<NearbyProviderController>().updateIsFavoriteValue(
          status, providerId,
          shouldUpdate: true, fromExploreProviderScreen: false);
    }
    if (shouldUpdate) {
      update();
    }
  ...
  updateServiceIsFavoriteValue(int status, String serviceId,
    ...
    if (_providerDetailsContent != null &&
      ...
      for (int categoryIndex = 0;
        ...
        int? serviceIndex = _providerDetailsContent
            ?.subCategories![categoryIndex].services
            ?.indexWhere((element) => element.id == serviceId);
        if (serviceIndex != null && serviceIndex > -1) {
          _providerDetailsContent?.subCategories?[categoryIndex]
              .services?[serviceIndex].isFavorite = status;
        }
      }
    }
    if (shouldUpdate) {
  ...
  Future<void> setMarker(
      GoogleMapController mapController, LatLng initialPosition) async {
    final Uint8List selectedProvider =
        await convertAssetToUnit8List(Images.selectedProvider, width: 70);
    final Uint8List unselectedProvider =
        await convertAssetToUnit8List(Images.unselectedProvider, width: 60);
    final Uint8List currentLocationIcon =
        await convertAssetToUnit8List(Images.currentLocation, width: 50);
    ...
    for (int index = 0; index < _providerList!.length; index++) {
      if (_providerList![index].coordinates != null) {
        markers.add(Marker(
            onTap: () async {
              _resetMarker(index, mapController, initialPosition,
                  selectedProvider, unselectedProvider, currentLocationIcon);
              await scrollController!.scrollToIndex(index,
                  preferPosition: AutoScrollPosition.middle);
              await scrollController!.highlight(index);
            },
            markerId: MarkerId('branch_$index'),
            position: LatLng(_providerList![index].coordinates!.latitude!,
  ...
  void mapBound(GoogleMapController controller) async {
    ...
    for (int index = 0; index < _providerList!.length; index++) {
      if (_providerList![index].coordinates != null) {
        latLongList.add(LatLng(_providerList![index].coordinates!.latitude!,
            _providerList![index].coordinates!.longitude!));
      }
    }

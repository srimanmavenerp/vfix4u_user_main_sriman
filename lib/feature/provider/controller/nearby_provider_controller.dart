...
class NearbyProviderController extends GetxController implements GetxService {
  ...
  Future<void> getProviderList(int offset, bool reload,
      {bool applyFilter = false, LatLng? initialPosition}) async {
    if (offset != 1 || _providerModel == null || reload) {
      ...
      if (offset == 1) {
        await DataSyncHelper.fetchAndSyncData(
          fetchFromLocal: () => providerBookingRepo
              .getProviderList<CacheResponseData>(offset, body,
                  source: DataSourceEnum.local, limit: 100),
          fetchFromClient: () => providerBookingRepo.getProviderList(
              offset, body,
              source: DataSourceEnum.client, limit: 100),
        ...
        ApiResponseModel response = await providerBookingRepo.getProviderList(
            offset, body,
            limit: 100, source: DataSourceEnum.client);
        if (response.response.statusCode == 200) {
          ...
          if (_providerModel != null) {
            _providerList!.addAll(
                ProviderModel.fromJson(response.response).content?.data ?? []);
          }
          _sortProviderListAndInitMap(initialPosition: initialPosition);
        } else {
  ...
  Future<void> updateIsFavoriteStatus(
      {required String providerId, required int index}) async {
    _apiHitCount++;
    updateIsFavoriteValue(
        _providerList?[index].isFavorite == 1 ? 0 : 1, providerId);
    update();
    Response response =
  ...
  updateIsFavoriteValue(int status, String providerId,
      {bool shouldUpdate = false, bool fromExploreProviderScreen = true}) {
    int? index =
    ...
    if (fromExploreProviderScreen) {
      Get.find<ProviderBookingController>().updateProviderIsFavoriteValue(
          status, providerId,
          shouldUpdate: true, fromProviderBooking: false);
    }
    if (shouldUpdate) {
      update();
    }
  ...
  Future<BitmapDescriptor> convertAssetToUnit8List(String imagePath,

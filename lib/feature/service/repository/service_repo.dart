import 'package:Vfix4u/common/enums/enums.dart';
...
class ServiceRepo extends DataSyncRepo {
  ...
  Future<ApiResponseModel<T>> getAllServiceList<T>(
    ...
    return await fetchData<T>(
        '${AppConstants.allServiceUri}?offset=$offset&limit=10', source);
  }

  Future<ApiResponseModel<T>> getPopularServiceList<T>(
      {required DataSourceEnum source, int offset = 1}) async {
    return await fetchData<T>(
        '${AppConstants.popularServiceUri}?offset=$offset&limit=10', source);
  }

  Future<ApiResponseModel<T>> getTrendingServiceList<T>(
      {required DataSourceEnum source, int offset = 1}) async {
    return await fetchData<T>(
        '${AppConstants.trendingServiceUri}?offset=$offset&limit=10', source);
  }
